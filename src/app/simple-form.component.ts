...
export class SimpleFormComponent implements OnInit, OnDestroy {
      ...
      .pipe(
        tap(
          (response) => {
            ...
            this.form.get('title')?.patchValue(this.titleOptions.find((item) => item.isDefault));
          }
        ),
  ...
  public invalidLastName(): Boolean {
    return (this.submitted || this.form.controls['lastName'].touched) && this.form.controls['lastName'].hasError('required');
  }
